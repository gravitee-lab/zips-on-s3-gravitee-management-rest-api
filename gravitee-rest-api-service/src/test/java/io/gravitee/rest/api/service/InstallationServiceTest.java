...
public class InstallationServiceTest {
    ...
    public void shouldUpdateAdditionalInformation() throws TechnicalException {
        ...
        updatedInstallation.setUpdatedAt(Date.from(Instant.now()));
        when(installationRepository.update(any())).thenReturn(updatedInstallation);

        final InstallationEntity updatedInstallationEntity = installationService.setAdditionalInformation(newAdditionalInformation);

        verify(installationRepository).find();

        ArgumentCaptor<Installation> installationToUpdateCaptor = ArgumentCaptor.forClass(Installation.class);

        verify(installationRepository).update(installationToUpdateCaptor.capture());

        final Installation installationToUpdateCaptorValue = installationToUpdateCaptor.getValue();
        assertEquals(INSTALLATION_ID, installationToUpdateCaptorValue.getId());
        assertEquals(installation.getCreatedAt(), installationToUpdateCaptorValue.getCreatedAt());
        assertTrue(installationToUpdateCaptorValue.getUpdatedAt().after(now));
        assertEquals(newAdditionalInformation , installationToUpdateCaptorValue.getAdditionalInformation());

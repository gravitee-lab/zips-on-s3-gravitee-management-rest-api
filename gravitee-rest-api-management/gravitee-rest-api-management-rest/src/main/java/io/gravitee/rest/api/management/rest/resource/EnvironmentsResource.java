...
public class EnvironmentsResource extends AbstractResource {
    ...
    @ApiResponses({
            @ApiResponse(code = 200, message = "Current user permissions on its environements", response = char[].class, responseContainer = "Map"),
            @ApiResponse(code = 500, message = "Internal server error")})
    public Collection<EnvironmentPermissionsEntity> getEnvironmentsPermissions(
            @ApiParam("To filter on environment id") @QueryParam("id") String id,
            @ApiParam("To filter on environment hrid") @QueryParam("hrid") String hrid
    ) {
        List<EnvironmentEntity> environments = this.environmentService.findByUser(getAuthenticatedUserOrNull(), id, hrid);
